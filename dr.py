...
def pca(X, K):
    ...
    #             
    ### TODO: YOUR CODE HERE
    XTX = dot(X.T,X) / (N-1) # D*D: (D*N) * (N*D)
    evals,evecs = linalg.eig(XTX)
    evals = real(evals)      # D*1
    evecs = real(evecs)      # D*D
    order = argsort(-evals)  # negate for descending order
    evals = evals[order[:K]] # K*1: largest K eigenvalues
    Z = evecs[:,order[:K]]   # D*K: first K eigenvectors
    P = dot(X, Z)            # N*K: (N*D) * (D*K)
    # util.raiseNotDefined()
